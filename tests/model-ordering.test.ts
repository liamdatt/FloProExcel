...
void test("default model picker returns a curated OpenRouter model", () => {
  const model = pickDefaultModel([OPENROUTER_PROVIDER]);
  assert.equal(model.provider, OPENROUTER_PROVIDER);
  assert.equal(OPENROUTER_CURATED_MODELS.includes(model.id), true);
});
