...
void test("extensions hub connections tab includes MCP test flow", async () => {
  ...
  assert.match(source, /label: "Managed MCP servers"/);
  assert.match(source, /label: "Custom MCP servers"/);
  assert.match(source, /\+ Add server/);
  assert.match(source, /setManagedMcpServerEnabled/);
...
void test("taskpane init no longer wires proxy banner in managed mode", async () => {
  const initSource = await readFile(new URL("../src/taskpane/init.ts", import.meta.url), "utf8");

  assert.doesNotMatch(initSource, /createProxyBanner/);
  assert.doesNotMatch(initSource, /pi:proxy-state-changed/);
  assert.doesNotMatch(initSource, /proxyBanner\.update/);
});
...
void test("extensions hub groups connections, plugins, and skills with tabs", async () => {
  ...
  assert.match(hubSource, /Connections, plugins, and skills that extend FloPro/);
...
void test("settings overlay serializes open flow and adopts managed access + execution controls", async () => {
  ...
  assert.match(settingsOverlaySource, /buildManagedAccessSection/);
  assert.match(settingsOverlaySource, /Managed model access/);
  assert.match(settingsOverlaySource, /createToggleRow/);
  assert.match(settingsOverlaySource, /"Auto mode"/);
  assert.doesNotMatch(settingsOverlaySource, /validateOfficeProxyUrl/);
