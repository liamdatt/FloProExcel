...
function renderStatusBar(
  ...
  let ctxColor = "";
  const ctxBaseTooltip = `How much of FloPro's memory (context window) the conversation is using — ${totalTokens.toLocaleString()} / ${contextWindow.toLocaleString()} tokens. As this fills up, FloPro may get confused. Free up context with /compact or start a fresh chat with /new.`;
  let ctxWarning = "";
  let ctxWarningText = "";
  if (pct > 100) {
  ...
  const modeTooltip = modeIsAuto
    ? "Auto: FloPro applies workbook changes immediately. Click to switch to Confirm."
    : "Confirm: FloPro asks before each workbook change. Click to switch to Auto.";
  ...
  const thinkingTooltip = escapeAttr(
    "How deeply FloPro reasons before answering — higher is slower but more thorough. Click to choose, or ⇧Tab to cycle.",
  );

  el.innerHTML = `
    <button type="button" class="pi-status-model pi-status-clickable" data-tooltip="Switch the AI model powering this session">
      <span class="pi-status-model__mark">FP</span>
