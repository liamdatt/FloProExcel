...
export async function initTaskpane(opts: {
  ...
  const availableProviders = [OPENROUTER_PROVIDER];
  ...
  const createRuntime = async (optsForRuntime: {
    ...
    const initialModelCandidate = getActiveRuntime()?.agent.state.model ?? defaultModel;
    const initialModel = isOpenRouterCuratedModel(initialModelCandidate)
    ...
    // Managed OpenRouter key. Users never paste API keys in the add-in.
    agent.getApiKey = (provider: string) => {
      if (provider === OPENROUTER_PROVIDER) {
        clearErrorBanner(errorRoot);
        return Promise.resolve(MANAGED_OPENROUTER_API_KEY_SENTINEL);
      }

      showErrorBanner(errorRoot, `Unsupported provider "${provider}". FloPro only uses managed OpenRouter.`);
    ...
    const unsubscribeErrorTracking = agent.subscribe((ev) => {
      ...
      if (agent.state.error) {
        ...
        if (!isAbort) {
          showErrorBanner(errorRoot, `LLM error: ${agent.state.error}`);
        }
      } else {
        clearErrorBanner(errorRoot);
      }
  ...
  const restorePersistedTabLayout = async (): Promise<SessionRuntime | null> => {
    ...
    for (const sessionId of savedLayout.sessionIds) {
      ...
      if (sessionData) {
        ...
        if (!isOpenRouterCuratedModel(runtime.agent.state.model)) {
          runtime.agent.setModel(defaultModel);
        }
      } else {
  ...
  const closeRuntimeWithRecovery = async (
    ...
    if (runtime.agent.state.isStreaming) {
      const proceed = await requestConfirmationDialog({
        ...
        message: "FloPro is still responding in this tab. Stop and close it?",
  ...
  const duplicateRuntimeTab = async (runtimeId: string): Promise<void> => {
    ...
    if (sourceModel && isOpenRouterCuratedModel(sourceModel)) {
      duplicateRuntime.agent.setModel(sourceModel);
    } else {
      duplicateRuntime.agent.setModel(defaultModel);
    }
  ...
  const onCommandRun: EventListener = (event) => {
    ...
    if (busy && !isBusyAllowedCommand(name)) {
      showToast(`Can't run /${name} while FloPro is busy`);
      return;
    }
