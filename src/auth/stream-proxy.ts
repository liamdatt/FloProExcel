/**
 * Stream wiring for the Office taskpane.
 *
 * FloPro uses managed OpenRouter only and routes requests through the same-origin
 * backend endpoint (`/api/openrouter/v1`).
 */
...
export function normalizeManagedBaseUrl(
  ...
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Managed OpenRouter base URL must be http(s) or same-origin path.");
  }

  return parsed.toString().replace(/\/+$/u, "");
}

function applyManagedOpenRouterBaseUrl(model: Model<Api>, rawBaseUrl: string): Model<Api> {
  if (model.provider !== OPENROUTER_PROVIDER) {
    throw new Error(
      `Unsupported provider \"${model.provider}\". FloPro uses managed OpenRouter only.`,
    );
  }

  if (!isOpenRouterCuratedModelId(model.id)) {
    throw new Error(`Model \"${model.id}\" is not in the curated OpenRouter list.`);
  }

  return {
    ...model,
    baseUrl: normalizeManagedBaseUrl(rawBaseUrl),
  };
}
...
export function createOfficeStreamFn(getOpenRouterBaseUrl: GetOpenRouterBaseUrl) {
  return async (model: Model<Api>, context: Context, options?: StreamOptions) => {
    ...
    const openRouterBaseUrl = await getOpenRouterBaseUrl();
    const normalizedModel = applyManagedOpenRouterBaseUrl(model, openRouterBaseUrl);
