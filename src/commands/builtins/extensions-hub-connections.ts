...
export async function renderConnectionsTab(args: {
  ...
  const [
    ...
    loadManagedMcpServers(settings),
  ...
  const masterToggle = createToggleRow({
    ...
    sublabel: "Allow FloPro to search the web and call external services",
  ...
  // ── Managed MCP servers section ───────────────────────
  container.appendChild(createSectionHeader({ label: "Managed MCP servers" }));

  const managedMcpList = document.createElement("div");
  managedMcpList.className = "pi-hub-stack";

  if (managedMcpServers.length === 0) {
    managedMcpList.appendChild(createEmptyInline(lucide(Zap), "No managed MCP servers available."));
  } else {
    for (const server of managedMcpServers) {
      managedMcpList.appendChild(renderManagedMcpServerCard(server, settings, isBusy, runMutation));
    }
  }
  container.appendChild(managedMcpList);
...
// ── Managed MCP server card ─────────────────────────

function renderManagedMcpServerCard(
  ...
  const card = createItemCard({
    ...
    expandable: true,
    badges: [
      getStatusBadge(server.enabled, toolLabel),
      { text: "Managed", tone: "muted" },
    ],
  });
  ...
  const enabledToggle = createToggle({
    ...
    onChange: (checked) => {
      const managedId = server.managedId;
      if (!managedId) return;
      void runMutation(async () => {
  ...
  card.body.appendChild(createActionsRow(testBtn));
  return card.root;
}
