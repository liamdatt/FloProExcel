...
function modeDescription(mode: ExecutionMode): string {
  if (mode === "yolo") {
    return `${formatExecutionModeLabel(mode)} mode — FloPro applies workbook changes immediately.`;
  }

  return `${formatExecutionModeLabel(mode)} mode — FloPro asks before each workbook change.`;
}
...
export function createSettingsCommands(actions: SettingsCommandActions): SlashCommand[] {
  return [
    {
      name: "settings",
      description: "Settings (managed access and advanced options)",
      source: "builtin",
      execute: () => {
    ...
    {
      name: "login",
      description: "Open managed access settings",
      source: "builtin",
      execute: async () => {
        await showSettingsDialog({ section: "logins" });
        showToast("Managed OpenRouter is already configured. No provider login is required.");
      },
    },
    {
      name: "yolo",
    ...
    {
      name: "rules",
      description: "Edit rules for FloPro (all files + this file)",
      source: "builtin",
      execute: async () => {
