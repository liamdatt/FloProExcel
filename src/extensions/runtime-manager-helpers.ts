...
export function resolveModelForCompletion(args: {
  ...
  const curatedAvailable = getModels(OPENROUTER_PROVIDER)
    .filter((model) => isOpenRouterCuratedModelId(model.id));

  const defaultModel = curatedAvailable[0];
  if (!defaultModel) {
    throw new Error(
      `No curated OpenRouter models are available. Expected one of: ${OPENROUTER_CURATED_MODELS.join(", ")}`,
    );
  }

  if (!requestedModel || requestedModel.trim().length === 0) {
    if (fallbackModel.provider === OPENROUTER_PROVIDER && isOpenRouterCuratedModelId(fallbackModel.id)) {
  ...
  const openRouterPrefix = `${OPENROUTER_PROVIDER}/`;
  const requestedId = trimmed.startsWith(openRouterPrefix)
    ? trimmed.slice(openRouterPrefix.length)
    : trimmed;

  if (!isOpenRouterCuratedModelId(requestedId)) {
    throw new Error(`Model is not allowed. Choose one of: ${OPENROUTER_CURATED_MODELS.join(", ")}`);
  }

  const match = curatedAvailable.find((model) => model.id === requestedId);
  if (!match) {
    throw new Error(`Configured model is unavailable in this runtime: ${requestedId}`);
  }
